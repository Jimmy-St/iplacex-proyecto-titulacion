...
<div class="relative">
    <button @click="if (status !== 'COMPLETADO') showStatusDropdown = !showStatusDropdown" 
        ...
        <div class="flex items-center gap-2">
            ...
            <span x-text="status === 'COMPLETADO' ? 'A PAGAR' : status"></span>
        </div>
    ...
    <div x-show="showStatusDropdown && status !== 'COMPLETADO'" 
        ...
        <button @click="changeStatus('COMPLETADO')" class="w-full text-left px-3 py-2 rounded-lg text-xs font-semibold text-emerald-800 bg-emerald-50/60 hover:bg-emerald-100/80 transition-colors">A PAGAR</button>
    </div>
</div>